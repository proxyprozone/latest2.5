...
export default function AdminDashboard() {
  ...
  const handleProductImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      try {
        ...
        setNewProduct({ ...newProduct, image_url: compressedBase64 });
      } catch (error) {
        console.error("Error compressing image:", error);
        setSuccessMessage("Error uploading image. Please try a smaller file.");
      }
    }
  };
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>, field: 'qr_image_url' | 'logo_url' | 'bg_image_url' | 'products_bg_image_url' | 'about_us_image_url') => {
    const file = e.target.files?.[0];
    if (file) {
      try {
        const compressedBase64 = await compressImage(file);
